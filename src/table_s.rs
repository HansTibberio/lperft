...
impl PackedData {
    #[inline(always)]
    fn read(&self) -> HashEntry {
        let zobrist: u64 = self.zobrist;
        let data: u64 = self.data;
        HashEntry::from((zobrist, data))
    }

    #[inline(always)]
    fn write(&mut self, entry: HashEntry) {
...
impl HashTable {
    ...
    #[inline(always)]
    pub fn probe(&self, hash: Zobrist) -> HashEntry {
        unsafe { self.table.get_unchecked(self.get_index(hash)).read() }
    }

    #[inline(always)]
    pub fn add(&mut self, zobrist: Zobrist, nodes: u64, depth: u8) {
        let index: usize = self.get_index(zobrist);
        let old_slot: &mut PackedData = unsafe { self.table.get_unchecked_mut(index) };
        let old: HashEntry = old_slot.read();
